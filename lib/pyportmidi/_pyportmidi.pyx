...
class Input:
    ...
    def __dealloc__(self):
        cdef PmError err
        if self.debug: print("Closing MIDI input stream and destroying instance", self.midi == NULL)
...
Close()
    ...
    exits -- this is particularly difficult under Windows.)
        """
        print("Input.Close(): ", self.midi == NULL)
        self._check_open()
        err = Pm_Close(self.midi)
        if err < 0:
            raise Exception, Pm_GetErrorText(err)
        self.midi = NULL
        print("Input.Close() exited", self.midi == NULL)
